...
/* disco: per-env nh_g->s_o_init_c_disco */
...
d_level *dlev;
{
    ...
    for (j = 0; j < 9 - lev / 3; j++)
        NH_G(objects)[first + j].oc_prob = 0;
    first += j;
    if (first > LAST_GEM || NH_G(objects)[first].oc_class != GEM_CLASS
        || OBJ_NAME(NH_G(objects)[first]) == (char *) 0) {
        raw_printf("Not enough gems? - first=%d j=%d LAST_GEM=%d", first, j,
                   LAST_GEM);
        wait_synch();
    }
    for (j = first; j <= LAST_GEM; j++)
        NH_G(objects)[j].oc_prob = (171 + j - first) / (LAST_GEM + 1 - first);
}
...
boolean domaterial;
{
    ...
    for (num_to_shuffle = 0, j = o_low; j <= o_high; j++)
        if (!NH_G(objects)[j].oc_name_known)
            num_to_shuffle++;
    if (num_to_shuffle < 2)
        return;
    ...
    for (j = o_low; j <= o_high; j++) {
        ...
        do
            i = j + rn2(o_high - j + 1);
        while (NH_G(objects)[i].oc_name_known);
        sw = NH_G(objects)[j].oc_descr_idx;
        NH_G(objects)[j].oc_descr_idx = NH_G(objects)[i].oc_descr_idx;
        NH_G(objects)[i].oc_descr_idx = sw;
        sw = NH_G(objects)[j].oc_tough;
        NH_G(objects)[j].oc_tough = NH_G(objects)[i].oc_tough;
        NH_G(objects)[i].oc_tough = sw;
        color = NH_G(objects)[j].oc_color;
        NH_G(objects)[j].oc_color = NH_G(objects)[i].oc_color;
        NH_G(objects)[i].oc_color = color;

        /* shuffle material */
        if (domaterial) {
            sw = NH_G(objects)[j].oc_material;
            NH_G(objects)[j].oc_material = NH_G(objects)[i].oc_material;
            NH_G(objects)[i].oc_material = sw;
        }
    }
}
...
#define COPY_OBJ_DESCR(o_dst, o_src) o_dst.oc_descr_idx = o_src.oc_descr_idx
    ...
    for (i = 0; i < NUM_OBJECTS; i++)
        NH_G(objects)[i].oc_name_idx = NH_G(objects)[i].oc_descr_idx = i;
    /* init base; if probs given check that they add up to 1000,
       otherwise compute probs */
    first = 0;
    while (first < NUM_OBJECTS) {
        oclass = NH_G(objects)[first].oc_class;
        last = first + 1;
        while (last < NUM_OBJECTS && NH_G(objects)[last].oc_class == oclass)
            last++;
        ...
        if (oclass == GEM_CLASS) {
            ...
            if (rn2(2)) { /* change turquoise from green to blue? */
                COPY_OBJ_DESCR(NH_G(objects)[TURQUOISE], NH_G(objects)[SAPPHIRE]);
            }
            if (rn2(2)) { /* change aquamarine from green to blue? */
                COPY_OBJ_DESCR(NH_G(objects)[AQUAMARINE], NH_G(objects)[SAPPHIRE]);
            }
            switch (rn2(4)) { /* change fluorite from violet? */
            case 0:
                break;
            case 1: /* blue */
                COPY_OBJ_DESCR(NH_G(objects)[FLUORITE], NH_G(objects)[SAPPHIRE]);
                break;
            case 2: /* white */
                COPY_OBJ_DESCR(NH_G(objects)[FLUORITE], NH_G(objects)[DIAMOND]);
                break;
            case 3: /* green */
                COPY_OBJ_DESCR(NH_G(objects)[FLUORITE], NH_G(objects)[EMERALD]);
                break;
            }
        }
    check:
        ...
        if (sum == 0) {
            for (i = first; i < last; i++)
                NH_G(objects)[i].oc_prob = (1000 + i - first) / (last - first);
            goto check;
        }
        if (sum != 1000)
    ...
    shuffle_tiles();
#endif
    NH_G(objects)[WAN_NOTHING].oc_dir = rn2(2) ? NODIR : IMMEDIATE;
}
...
{
    int i, ocls = NH_G(objects)[otyp].oc_class;
    ...
    *lo_p = *hi_p = otyp;
    ...
    case SPBOOK_CLASS:
        ...
        *lo_p = bases[ocls];
        for (i = *lo_p; NH_G(objects)[i].oc_class == ocls; i++)
            if (NH_G(objects)[i].oc_unique || !NH_G(objects)[i].oc_magic)
                break;
        *hi_p = i - 1;
        break;
    case RING_CLASS:
...
find_skates()
{
    ...
    for (i = SPEED_BOOTS; i <= LEVITATION_BOOTS; i++)
        if ((s = OBJ_DESCR(NH_G(objects)[i])) != 0 && !strcmp(s, "snow boots"))
            return i;
...
int fd, mode;
{
    ...
    if (perform_bwrite(mode)) {
        ...
        bwrite(fd, (genericptr_t) NH_G(s_o_init_c_disco), sizeof NH_G(s_o_init_c_disco));
        bwrite(fd, (genericptr_t) NH_G(objects),
    ...
    for (i = 0; i < NUM_OBJECTS; i++)
        if (NH_G(objects)[i].oc_uname) {
            if (perform_bwrite(mode)) {
                len = strlen(NH_G(objects)[i].oc_uname) + 1;
                bwrite(fd, (genericptr_t) &len, sizeof len);
                bwrite(fd, (genericptr_t) NH_G(objects)[i].oc_uname, len);
            }
            if (release_data(mode)) {
                free((genericptr_t) NH_G(objects)[i].oc_uname);
                NH_G(objects)[i].oc_uname = 0;
            }
        }
}
...
register int fd;
{
    ...
    mread(fd, (genericptr_t) NH_G(s_o_init_c_disco), sizeof NH_G(s_o_init_c_disco));
    mread(fd, (genericptr_t) NH_G(objects), sizeof(struct objclass) * NUM_OBJECTS);
    for (i = 0; i < NUM_OBJECTS; i++)
        if (NH_G(objects)[i].oc_uname) {
            mread(fd, (genericptr_t) &len, sizeof len);
            NH_G(objects)[i].oc_uname = (char *) alloc(len);
            mread(fd, (genericptr_t) NH_G(objects)[i].oc_uname, len);
        }
#ifdef USE_TILES
    shuffle_tiles();
#endif
...
boolean credit_hero;
{
    if (!NH_G(objects)[oindx].oc_name_known) {
        register int dindx, acls = NH_G(objects)[oindx].oc_class;
         ...
         */
        for (dindx = bases[acls]; NH_G(s_o_init_c_disco)[dindx] != 0; dindx++)
            if (NH_G(s_o_init_c_disco)[dindx] == oindx)
                break;
        NH_G(s_o_init_c_disco)[dindx] = oindx;

        if (mark_as_known) {
            NH_G(objects)[oindx].oc_name_known = 1;
            if (credit_hero)
                exercise(A_WIS, TRUE);
        }
        /* moves==1L => initial inventory, gameover => final disclosure */
        if (moves > 1L && !NH_G(program_state).gameover) {
            if (NH_G(objects)[oindx].oc_class == GEM_CLASS)
...
register int oindx;
{
    if (!NH_G(objects)[oindx].oc_name_known) {
        ...
        for (dindx = bases[acls]; dindx < NUM_OBJECTS && NH_G(s_o_init_c_disco)[dindx] != 0
                                  && NH_G(objects)[dindx].oc_class == acls;
             dindx++)
            if (found)
                NH_G(s_o_init_c_disco)[dindx - 1] = NH_G(s_o_init_c_disco)[dindx];
            else if (NH_G(s_o_init_c_disco)[dindx] == oindx)
                found = TRUE;

        /* clear last slot */
        if (found)
            NH_G(s_o_init_c_disco)[dindx - 1] = 0;
        else
...
register int i;
{
    ...
    return (boolean) (NH_G(objects)[i].oc_uname != (char *) 0
                      || (NH_G(objects)[i].oc_name_known
                          && OBJ_DESCR(NH_G(objects)[i]) != (char *) 0));
}
...
dodiscovered() /* free after Robert Viduya */
{
    ...
    for (i = dis = 0; i < SIZE(uniq_objs); i++)
        if (NH_G(objects)[uniq_objs[i]].oc_name_known) {
            if (!dis++)
                putstr(tmpwin, iflags.menu_headings, "Unique items");
            Sprintf(buf, "  %s", OBJ_NAME(NH_G(objects)[uniq_objs[i]]));
            putstr(tmpwin, 0, buf);
            ++ct;
        }
    ...
    Strcpy(classes, NH_G(flags).inv_order);
    ...
    for (s = classes; *s; s++) {
        ...
        for (i = bases[(int) oclass];
             i < NUM_OBJECTS && NH_G(objects)[i].oc_class == oclass; i++) {
            if ((dis = NH_G(s_o_init_c_disco)[i]) != 0 && interesting_to_discover(dis)) {
                ct++;
                ...
                Sprintf(buf, "%s %s",
                        (NH_G(objects)[dis].oc_pre_discovered ? "*" : " "),
                        obj_typename(dis));
                putstr(tmpwin, 0, buf);
            }
        }
...
doclassdisco()
{
    ...
    discosyms[0] = '\0';
    traditional = (NH_G(flags).menu_style == MENU_TRADITIONAL
    ...
    Strcpy(allclasses, NH_G(flags).inv_order);
    ...
    if (traditional) {
        ...
        if (!discosyms[1] && NH_G(flags).menu_style == MENU_PARTIAL) {
...
rename_disco()
{
    ...
    for (s = NH_G(flags).inv_order; *s; s++) {
        oclass = *s;
        prev_class = oclass + 1; /* forced different from oclass */
        for (i = bases[(int) oclass];
             i < NUM_OBJECTS && NH_G(objects)[i].oc_class == oclass; i++) {
            dis = NH_G(s_o_init_c_disco)[i];
    ...
    if (ct == 0) {
        ...
        if (dis != STRANGE_OBJECT) {
            ...
            odummy = zeroobj;
            odummy.otyp = dis;
            odummy.oclass = NH_G(objects)[dis].oc_class;
            odummy.quan = 1L;
            odummy.known = !NH_G(objects)[dis].oc_uses_known;
            odummy.dknown = 1;
            docall(&odummy);
        }
    }
